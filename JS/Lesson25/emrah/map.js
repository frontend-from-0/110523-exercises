...
const squaredNumbers = numbersExercise2.map((num) => num * num);
...
const wordsInSentences = sentencesExercise7.map((sentence) =>
  sentence.split(" ")
);
...
const bookTitlesAndAuthors = booksExercise9.map(
...
const priceWithCurrency = pricesExercise10.map((price) => `$${price / 100}`);
