...
const usersAgeFiltered = usersExercise4.filter((user) => user.age >= 25);
console.log("usersAgeFiltered", usersAgeFiltered);
...
const sentencesWordsFiltered = sentencesExercise5.filter(
  (sentence) => sentence.split(" ").length > 10
);
console.log("sentencesWordsFiltered", sentencesWordsFiltered);
...
const productsInStock = productsExercise6.filter(
...
const filteredNumbers = numbersExercise7.filter((number) => number % 3 !== 0);
...
const filteredStudents = studentsExercise9.filter(
  (student) => student.grade[0].toUpperCase() != "D"
);
...
const filteredUsers = usersExercise10.filter(
