...
const Ferrari = new Car('ferrari', 1950, 2024);
const Tesla = new Car('tesla', 2020, 2025, 4);
...
//with properties for type (either "deposit" or "withdrawal"), amount, and date ({type: "withdrawal", amount: amount, date: new Date().toLocaleDateString()}). 
//Implement deposit() and withdraw() methods that add new transactions to the transactionHistory array.Implement a get transactionHistory() 
//getter method that returns a copy of the transaction history array, and a get currentBalance() getter method that calculates and returns 
//the current balance of the account based on the transaction history.
...
class BankAccount {
    ...
    deposit(amount) {
        if (amount > 0) {
            ...
            return this._transactionHistory.push({
                type: 'deposit',
                amount: amount,
    ...
    withdraw(amount) 
    {
        ...
        console.log('Unable to process withdrawal. Please enter a positive amount smaller than current balance.')};
      }
